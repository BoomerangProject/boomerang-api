...
func BoomerangContract() {
	...
	jsonFile, _ := ioutil.ReadFile("boomerangTokenContract.json")

	json.Unmarshal(jsonFile, &jsonObject)
	contractAbi := getAbiFromString(string(jsonObject.Abi))

	boomerangTokenContractAddress := parameters.GetBoomerangTokenAddress()

	boomerangTokenContract := bind.NewBoundContract(boomerangTokenContractAddress, contractAbi, client, client, client)

	totalSupply := TotalSupply(boomerangTokenContract)
